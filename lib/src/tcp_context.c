...
int do_receive(tcp_context *tcp_ctx, int idx) {
  client_info *c_info = &tcp_ctx->ci_list[idx];
...
int socev_service(void *ctx, int timeout_ms) {
  ...
  if (ctx) {
    ...
    pollfd_list list = client_info_pollfd_list(&tcp_ctx->ci_list);
    list.fd_list[list.count].fd = tcp_ctx->fd;
    list.fd_list[list.count++].events = POLLIN;

    result = poll(list.fd_list, list.count, timeout_ms);
    ...
    if (result > 0) {
      for (int idx = 1; idx < list.count; idx += 2) {
      ...
      if (list.fd_list[list.count - 1].revents & POLLIN) {
